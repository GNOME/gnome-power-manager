...
/* THIS IS A BODGE UNTIL WE CHECK FOR CAIRO */
#define USE_CAIRO	FALSE
...
draw_grid (GpmSimpleGraph *graph, cairo_t *cr)
{
	...
	cairo_set_line_width (cr, 0.4 * cairo_get_line_width (cr));
	...
	for (a=1; a<10; a++) {
		...
		cairo_move_to (cr, b, graph->priv->box_y);
		cairo_line_to (cr, b, graph->priv->box_y + graph->priv->box_height);		
		cairo_stroke (cr);
	}
	...
	for (a=1; a<10; a++) {
		...
		cairo_move_to (cr, graph->priv->box_x, b);
		cairo_line_to (cr, graph->priv->box_x + graph->priv->box_width, b);		
		cairo_stroke (cr);
	}
