				    ...
				    long data1, long data2, long data3)
{
	...
	gdk_error_trap_push ();
	XSendEvent (display,
		    icon->manager_window, False, NoEventMask,
		    (XEvent *) & ev);
	XSync (display, False);
	gdk_error_trap_pop ();
}
...
egg_tray_icon_new_for_screen (GdkScreen * screen, const char *name)
{
	...
	return g_object_new (EGG_TYPE_TRAY_ICON, "screen", screen, "title",
			     name, NULL);
}
...
{
	g_return_val_if_fail (EGG_IS_TRAY_ICON (icon),
			      GTK_ORIENTATION_HORIZONTAL);
