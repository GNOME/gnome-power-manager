	      ...
	      int         icon_policy)
{
	...
	present = gpm_power_get_battery_status (manager->priv->power, BATT_TYPE_UPS, &status);
	...
	present = gpm_power_get_battery_status (manager->priv->power, BATT_TYPE_MOUSE, &status);
	present &= status.is_present;
	...
	present = gpm_power_get_battery_status (manager->priv->power, BATT_TYPE_KEYBOARD, &status);
	present &= status.is_present;
				    ...
				    BatteryStatus      *battery_status)
{
	...
	if (battery_status->percentage_charge >= 100 && battery_type == BATT_TYPE_PRIMARY 
	...
	if (battery_type == BATT_TYPE_PRIMARY && battery_status->is_charging) {
		manager->priv->done_warning_critical = FALSE;
		manager->priv->done_warning_very_low = FALSE;
		manager->priv->done_warning_low = FALSE;
	}

	if (battery_type == BATT_TYPE_PRIMARY && battery_status->is_discharging) {
		manager->priv->done_notification_fully_charged = FALSE;
	}

	if (! battery_status->is_discharging || battery_type != BATT_TYPE_PRIMARY) {
				...
				GpmManager         *manager)
{
	...
	gpm_power_get_battery_status (manager->priv->power, battery_type, &battery_status);

	maybe_notify_battery_power_changed (manager, battery_type, &battery_status);
